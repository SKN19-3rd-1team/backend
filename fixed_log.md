...
### 1. 대학 입시 정보 조회 기능 추가
- **새로운 툴 구현 (`backend/rag/tools.py`)**: `get_university_admission_info` 툴 추가
  - 대학명(필수)과 학과명(선택)을 입력받아 KCUE 입시 정보 URL 반환
  - 안내 메시지("입시제도에 대해서는...") 자동 포함
- **유틸리티 모듈 생성 (`backend/rag/university_lookup.py`)**:
  - `university_data_cleaned.json` 로드 및 검색 로직 분리
  - 대학명 부분 매칭 및 자동 보정("[본교]" 추가 등) 지원
- **에이전트 연동 (`backend/graph/nodes.py`)**:
  - 새로운 툴을 `tools` 리스트에 등록하고 시스템 프롬프트에 사용 가이드 추가

### 2. 전공 추천 정확도 개선 (줄임말 처리)
- **문제 해결**: "컴공", "화공" 등 줄임말 입력 시 정확한 전공("컴퓨터공학과")을 찾지 못해 선호 전공 가산점(5배)이 누락되는 문제 해결
- **LLM 기반 정규화 (`backend/graph/nodes.py`)**:
  - `_normalize_majors_with_llm` 함수 구현: LLM을 사용하여 사용자 입력을 "표준 학과명"으로 변환
  - `recommend_majors_node` 개선: 원본 입력과 정규화된 이름을 모두 사용하여 검색 정확도 극대화

### 3. 배포 안정성 및 이식성 확보
- **경로 문제 해결**: 로컬 절대 경로(`/home/maroco/...`)를 제거하고 `Path(__file__).parent` 기반의 동적 상대 경로로 변경
  - `backend/rag/tools.py` 및 `backend/rag/university_lookup.py` 적용
  - 어떤 환경(클라우드, 타 개발자 머신)에서도 데이터 파일을 안정적으로 로드 가능

### 4. 문서화 강화 (`README.md`)
- **데이터 구조 설명 추가**: Pinecone에 저장되는 5가지 문서 타입(`summary`, `interest`, `property`, `subjects`, `jobs`)에 대한 상세 설명 및 예시 추가
- **메타데이터 구조 명시**: 각 문서 타입별 저장 내용과 활용 방안을 표로 정리하여 이해도 향상
