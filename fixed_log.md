...
### 1. 검색 가이드 및 문서 최신화
- **검색 가이드 개선 (`backend/rag/tools.py`)**:
  - `get_search_help` 툴의 응답 메시지를 사용자 친화적인 어조로 전면 수정
  - **카테고리화**: 전공 탐색, 개설 대학 찾기, 진로 정보 등 3가지 카테고리로 나누어 구체적 예시 제공
  - **팁 추가**: 구체적인 질문을 유도하는 팁 문구 추가

- **시스템 문서 업데이트 (`nodes.md`)**:
  - **신규 툴 반영**: `get_major_career_info` 툴 설명 추가
  - **검색 로직 명시**: 4단계 검색 전략(정확 일치 → 별칭 → 벡터 → 토큰) 반영
  - **아키텍처 최신화**: 최신 툴 구조를 반영하여 시스템 아키텍처 다이어그램 수정
